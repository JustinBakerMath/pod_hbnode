...
class dfwrapper(nn.Module):
    def __init__(self, df, shape, recf=None, device='cpu'):
    ...
    def forward(self, t, x):
        ...
        if self.recf:
            ...
            dx = torch.cat([dx, dr], dim=1).to(self.device)
        else:
...
class temprnn(nn.Module):
    def __init__(self, in_channels, out_channels, nhidden, res=False, cont=False,device='cpu'):
        super().__init__()
        self.actv = nn.Tanh()
    ...
    def forward(self, h, x):
        out = torch.cat([h[:, 0], h[:, 1], x], dim=1).to(self.device)
        out = self.dense1(out)
        out = self.actv(out)
        #out = self.dense2(out)
        #out = self.actv(out)
...
class nodernn(nn.Module):
    def __init__(self, in_channels, out_channels, nhidden,device='cpu'):
        super().__init__()
        self.actv = nn.Tanh()
    ...
    def forward(self, h, x):
        out = torch.cat([h, x], dim=1).to(self.device)
        out = self.dense1(out)
        out = self.actv(out)
        #out = self.dense2(out)
        #out = self.actv(out)
...
class NODE(nn.Module):
    def __init__(self, df=None, device='cpu', **kwargs):
    ...
    def forward(self, t, x):
        self.nfe += 1
        self.A = self.df.dense1.weight #stiffness 
        self.eig = np.sort(np.abs(np.real(np.linalg.eig(self.A.detach().cpu().numpy())[0])))
        self.stiff = self.eig[-1]/self.eig[0]
        self.A.to(self.device)
...
class HeavyBallNODE(NODE):
    def __init__(self, df, actv_h=None, gamma_guess=-3.0, gamma_act='sigmoid', corr=-100, corrf=True,device='cpu'):
    ...
    def forward(self, t, x):
        ...
        self.A = self.df.dense1.weight
        self.get_gamma()
        self.Beig = np.append((self.g + np.sqrt(self.g **2 + 4*self.eig))/2,  (self.g  - np.sqrt(self.g **2 + 4*self.eig))/2) #Stiffness stuff
        self.Beig = np.sort(np.abs(np.real(self.Beig)))
        self.stiff = self.Beig[-1]/self.Beig[0]
        ...
        out = torch.cat((dh, dm), dim=1).to(self.device)
        if self.elem_t is None:
            return out
        else:
...
class NMODEL(nn.Module):
    def __init__(self, args):
        ...
        self.rnn = nodernn(modes, nhid, nhid,device=DEVICE)
        self.ode_rnn = ODE_RNN_with_Grad_Listener(self.cell, self.rnn, nhid, None, rnn_out=True, tol=1e-7,device=DEVICE)
...
class HBMODEL(nn.Module):
    def __init__(self,args,  res=False, cont=False):
        ...
        modes = args.modes
        nhid = modes*args.layers
        self.cell = HeavyBallNODE(tempf(nhid, nhid), corr=args.corr, corrf=True,device=DEVICE)
        self.rnn = temprnn(modes, nhid, nhid, res=res, cont=cont,device=DEVICE)
        self.ode_rnn = ODE_RNN_with_Grad_Listener(self.cell, self.rnn, (2, nhid), None, tol=1e-7,device=DEVICE)
...
class GHBMODEL(nn.Module):
    def __init__(self, args, res=False, cont=False):
        ...
        modes = args.modes
        nhid = modes*args.layers
        self.cell = HeavyBallNODE(tempf(nhid, nhid), corr=args.corr, corrf=False, actv_h=nn.Tanh(),device=DEVICE)
...
class NODEintegrate(nn.Module):
    def __init__(self, df, shape=None, tol=1e-5, adjoint=True, evaluation_times=None, recf=None,device='cpu'):
        super().__init__()
        self.df = dfwrapper(df, shape, recf,device=device) if shape else df
        self.tol = tol
        self.device = device
    ...
    def forward(self, x0):
        ...
        if self.shape:
            ...
            if self.recf:
                reczeros = torch.normallike(x0[:, :1]).to(self.device)
                reczeros = repeat(reczeros, 'b 1 -> b c', c=self.recf.osize)
                x0 = torch.cat([x0, reczeros], dim=1).to(self.device)
...
class ODE_RNN(nn.Module):
    def __init__(self, ode, rnn, nhid, ic, rnn_out=False, both=False, tol=1e-7,device='cpu'):
        super().__init__()
        self.device = device
        self.ode = ode
        self.t = torch.Tensor([0, 1]).to(self.device)
    ...
    def forward(self, t, x, multiforecast=None):
        n_t, n_b = t.shape
        h_ode = torch.zeros(n_t + 1, n_b, *self.nhid).to(self.device)
        h_rnn = torch.zeros(n_t + 1, n_b, *self.nhid).to(self.device)
        if self.ic:
...
class ODE_RNN_with_Grad_Listener(nn.Module):
    ...
    def forward(self, t, x, multiforecast=None, retain_grad=False):
        ...
        h_ode = [None] * (n_t + 1)
        h_rnn = [None] * (n_t + 1)
        h_ode[-1] = h_rnn[-1] = torch.zeros(n_b, *self.nhid).to(self.device)
        if self.ic:
            h_ode[0] = h_rnn[0] = self.ic(rearrange(x, 'b t c -> b (t c)')).view((n_b, *self.nhid))
        else:
            h_ode[0] = h_rnn[0] = torch.zeros(n_b, *self.nhid).to(self.device)
        if self.rnn_out:
            for i in range(n_t):
                self.ode.update(t[i])
                h_ode[i] = odeint(self.ode, h_rnn[i], self.t, atol=self.tol, rtol=self.tol)[-1]
        ...
        if multiforecast is not None:
            self.ode.update(torch.normallike((t[0])).to(self.device))
            forecast = odeint(self.ode, out[-1][-1], multiforecast * 1.0, atol=self.tol, rtol=self.tol)
            out = (*out, forecast)
        if retain_grad:
            self.h_ode = h_ode
